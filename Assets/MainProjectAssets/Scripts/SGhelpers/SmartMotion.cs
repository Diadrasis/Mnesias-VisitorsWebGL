...
namespace StaGeGames.SmartUI
{
    ...
    public class SmartMotion : MonoBehaviour
    {
        ...
        {
            if (ShowButton) ShowButton.onClick.RemoveAllListeners();
            if (HideButtons.Length > 0) foreach (Button btn in HideButtons) btn.onClick.RemoveAllListeners();
        }
        ...
        private void PrepareEventsForEditorUse()
        {
            ...
            {
                if (OnHideComplete.GetPersistentEventCount() > 0)
                {
                    OnHideComplete.SetPersistentListenerState(0, UnityEventCallState.EditorAndRuntime);
                }
            }
            if (OnHideStart.GetPersistentEventCount() > 0) OnHideStart.SetPersistentListenerState(0, UnityEventCallState.EditorAndRuntime);
            if (OnShowComplete.GetPersistentEventCount() > 0) OnShowComplete.SetPersistentListenerState(0, UnityEventCallState.EditorAndRuntime);
            if (OnShowStart.GetPersistentEventCount() > 0) OnShowStart.SetPersistentListenerState(0, UnityEventCallState.EditorAndRuntime);
        }
